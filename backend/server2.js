...
// Rate Limiting (Increased for development/diagnostics: 1000 requests per 15 minutes)
...
// Manim Conversion Endpoint
const { convertPythonToTypeScript } = require('./tools/manim-converter');
app.post('/api/convert-manim', (req, res) => {
    try {
        const pythonCode = req.body.code;
        if (!pythonCode) {
            return res.status(400).json({ error: 'No code provided' });
        }
        const tsCode = convertPythonToTypeScript(pythonCode);
        res.json({ code: tsCode });
    } catch (error) {
        console.error('Manim conversion error:', error);
